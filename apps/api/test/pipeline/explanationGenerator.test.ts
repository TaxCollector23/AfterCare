...
describe('generateExplanations', () => {
  it('caps confidence when the model cites a nonexistent line number', async () => {
    callJsonMock.mockResolvedValueOnce({
      explanations: [{ term: 'anticoagulant', plainText: 'A blood thinner.', sourceLines: [99], confidence: 90 }],
    });
    const result = await generateExplanations(makeOcr(['Start anticoagulant therapy']));
  ...
  it('defaults a missing explanations array to empty with full confidence', async () => {
    callJsonMock.mockResolvedValueOnce({});
    const result = await generateExplanations(makeOcr(['plain text, no jargon']));
