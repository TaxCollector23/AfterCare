...
function makeOcr(lines: string[]): OcrResult {
  const ocrLines = lines.map((text, idx) => ({ line: idx + 1, text, confidence: 95 }));
...
describe('runExtraction', () => {
  it('passes through a well-formed model response', async () => {
    ...
    const result = await runExtraction(makeOcr(['Take amoxicillin twice daily']));

    expect(result.success).toBe(true);
    expect(result.data?.medicationsText).toBe('1: Take amoxicillin twice daily');
