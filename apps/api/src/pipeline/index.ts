...
import type { PipelineEmitter, RecoveryPlan, StageResult } from '@discharge/shared-types';
...
function emitStage(emit: PipelineEmitter, stage: Parameters<PipelineEmitter>[0]['stage']) {
  return {
    started: () => emit({ stage, status: 'started' }),
    done: (result: StageResult<unknown>) =>
      emit({ stage, status: 'done', data: result.data, confidence: result.confidence }),
    error: (message: string) => emit({ stage, status: 'error', error: message }),
...
export async function runPipeline(
  ...
  const timelineResult = await buildTimeline(sections.timelineText, { medications, appointments }, ocr);
  ...
  const explainResult = await cacheExplanations(hash, () => generateExplanations(ocr));
