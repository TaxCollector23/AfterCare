...
const VALID_BUCKETS: TimelineEntry["bucket"][] = [
  "today",
  "tomorrow",
  "this-week",
  "later",
...
export async function buildTimeline(
  ...
  try {
    ...
    const validLines = fullOcr
      ? new Set(fullOcr.lines.map((l) => l.line))
      : null;
    ...
    const overall =
      result.length === 0
        ? 100
        : Math.round(
            result.reduce((sum, t) => sum + t.confidence, 0) / result.length,
          );
