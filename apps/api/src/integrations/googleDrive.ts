...
export async function completeDriveAuthorization(code: string, state: string) {
  ...
  if (!response.ok || !result.access_token) {
    throw new AppError(
      502,
      result.error_description ?? "Google token exchange failed",
      "DRIVE_OAUTH_FAILED",
    );
  }
...
export async function importDriveFile(userId: string, fileId: string) {
  ...
  if (!metadataResponse.ok)
    throw new AppError(
      502,
      "Could not read Drive file metadata",
      "DRIVE_IMPORT_FAILED",
    );
...
export async function backupDriveFile(
  ...
  if (!response.ok || !result.id)
    throw new AppError(502, "Drive backup failed", "DRIVE_BACKUP_FAILED");
