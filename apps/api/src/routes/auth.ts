...
authRouter.post("/register", async (req, res, next) => {
  try {
    ...
    if (!parsed.success) throw new AppError(400, "Valid email and 12+ character password required", "INVALID_INPUT");
...
authRouter.post("/login", async (req, res, next) => {
  try {
    const parsed = credentials.safeParse(req.body);
    if (!parsed.success) throw new AppError(400, "Valid credentials required", "INVALID_INPUT");
