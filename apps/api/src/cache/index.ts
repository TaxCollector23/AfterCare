  ...
  if (!connecting) {
    connecting = (async () => {
      try {
        ...
        console.error('[cache] Failed to connect to Redis, caching disabled:', err);
        return null;
      }
    })();
  }
...
export function cacheExtraction<T>(fileHash: string, compute: () => Promise<T>): Promise<T> {
  return cacheAside(extractionCacheKey(fileHash), SEVEN_DAYS_SECONDS, compute, isSuccessfulStageResult);
}

/** Cache-aside wrapper for the explanation-generation stage, keyed by file_hash. TTL 30d. Never caches a failed result. */
export function cacheExplanations<T>(fileHash: string, compute: () => Promise<T>): Promise<T> {
  return cacheAside(explanationCacheKey(fileHash), THIRTY_DAYS_SECONDS, compute, isSuccessfulStageResult);
}
