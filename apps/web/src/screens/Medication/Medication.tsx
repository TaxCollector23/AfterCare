  ...
  return (
    <div className="card divider-section">
      ...
      <div className="flex" style={{ marginTop: 8, flexWrap: "wrap" }}>
        ...
        <span className={`chip ${med.afternoon ? "active" : ""}`}>Afternoon</span>
      ...
      {open && (
        <div style={{ marginTop: 12, borderTop: "1px solid var(--color-divider)", paddingTop: 12 }}>
          {med.foodInstructions && <p className="gloss">Food: {med.foodInstructions}</p>}
          {med.sideEffects && med.sideEffects.length > 0 && (
            <p className="gloss">Possible side effects: {med.sideEffects.join(", ")}</p>
          )}
          {med.missedDoseInstructions && <p className="gloss">If you miss a dose: {med.missedDoseInstructions}</p>}
        </div>
      )}
    </div>
  );
...
export default function Medication() {
  return (
    <div>
      <h1>Medications</h1>
      <p className="gloss measure">From your own medication list — nothing here is guessed.</p>
      <RecoveryGate>
        {(data) =>
          data.medications.length === 0 ? (
            <EmptyState icon="ph-pill" title="No medications found" description="Your document didn't list any medications." />
          ) : (
            <>
